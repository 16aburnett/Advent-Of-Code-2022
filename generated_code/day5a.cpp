...
int main () {
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            // LHS
            // Variable declaration
            char __main__for__85__block__86__for__88__block__89__box;
            __rhs = stack.back ();
            stack.pop_back ();
            __main__for__85__block__86__for__88__block__89__box = static_cast<char>(static_cast<unsigned char>(__rhs));
            // Result of assignment
            stack.push_back (static_cast<long>(static_cast<unsigned char>(__main__for__85__block__86__for__88__block__89__box)));
            // Statement results can be ignored
            stack.pop_back ();
            // End Statement
    ...
    // LHS
    // Variable declaration
    long __main__for__90__i;
    __rhs = stack.back ();
    stack.pop_back ();
    __main__for__90__i = *reinterpret_cast<long*>(&__rhs);
    // Result of assignment
    stack.push_back (*reinterpret_cast<long*>(&__main__for__90__i));
    // We can ignore the init result
    stack.pop_back ();
    // Using an infinite loop so we can write a separate multi-line condition
    while (1)
    {
        ...
        // Pre-Increment
        {
            ...
            __rhs = stack.back ();
            stack.pop_back ();
            __main__for__90__i = __main__for__90__i + 1;
            __res = __main__for__90__i;
            stack.push_back (*reinterpret_cast<long*>(&__res));
        }
        // We can ignore the update result
        stack.pop_back ();
